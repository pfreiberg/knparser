...
public class ZobrazeniVbOracleDatabaseJdbcExporter extends
	...
	private List<Object> getPrimaryKeysValues(Object record) {
		...
		try {
			for (int i = 0; i < methodsName.size(); i++) {
				Class<?> c = Class
						.forName("cz.pfreiberg.knparser.domain.prvkykatastralnimapy.ZobrazeniVb");
				Method method = c.getDeclaredMethod(methodsName.get(i));
				primaryKeyValues.add(method.invoke((ZobrazeniVb) record));
			}
	...
	private void processRecord(ZobrazeniVb record) {
	...
	private void processHistoricalRecord(ZobrazeniVb record) {
