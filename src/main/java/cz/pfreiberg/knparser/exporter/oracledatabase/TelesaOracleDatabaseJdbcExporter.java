...
public class TelesaOracleDatabaseJdbcExporter extends
	...
	public TelesaOracleDatabaseJdbcExporter(
			List<Telesa> telesa,
	...
	private List<Object> getPrimaryKeysValues(Object record) {
		...
		try {
			for (int i = 0; i < methodsName.size(); i++) {
				Class<?> c = Class
						.forName("cz.pfreiberg.knparser.domain.vlastnictvi.Telesa");
				Method method = c.getDeclaredMethod(methodsName.get(i));
				primaryKeyValues.add(method.invoke((Telesa) record));
			}
	...
	private void processRecord(Telesa record) {
	...
	private void processHistoricalRecord(Telesa record) {
		String datumVzniku = VfkUtil.formatValueDatabase(record
				.getDatumVzniku());
