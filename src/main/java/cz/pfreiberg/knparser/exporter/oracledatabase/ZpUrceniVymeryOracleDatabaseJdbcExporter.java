...
public class ZpUrceniVymeryOracleDatabaseJdbcExporter extends
	...
	private void prepareStatement() {
		...
		for (ZpUrceniVymery record : zpUrceniVymery) {
	...
	private List<Object> getPrimaryKeysValues(Object record) {
		...
		try {
			for (int i = 0; i < methodsName.size(); i++) {
				Class<?> c = Class
						.forName("cz.pfreiberg.knparser.domain.nemovitosti.ZpUrceniVymery");
				Method method = c.getDeclaredMethod(methodsName.get(i));
				primaryKeyValues.add(method.invoke((ZpUrceniVymery) record));
			}
